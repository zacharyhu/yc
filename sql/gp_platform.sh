...
echo "insert into gp_platform.gp_data_daily_game(l_date,l_game_id,l_game_name,l_total_num,l_user_num,l_avg_num,l_game_points,l_cash_sum,l_source)
select l_date,'999','×¼Æ,'0','0','0',sum(game_points),'0','999' from datacenter.dailyreport_game_play where   l_gid not like '4%' and l_gid !='999' and l_date=DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 DAY),'%y%m%d')"|mysql -h10.48.179.112 -udc -pdc
